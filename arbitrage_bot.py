...
# ---------------------- config (from env) ----------------------
# Basic constants (unchanged)
BINANCE_P2P_URL = "https://p2p.binance.com/bapi/c2c/v2/friendly/c2c/adv/search"
ROWS_PER_REQUEST = 20
TIMEOUT = 10
MAX_SCAN_PAGES = 60        # internal safety cap
SLEEP_BETWEEN_PAGES = 0.09
SLEEP_BETWEEN_PAIRS = 0.05

# Telegram token/chat: prefer env vars for safety (same names as before)
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "")
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "")

# Duplicate-alert protection TTL (in seconds)
ALERT_TTL_SECONDS = int(os.getenv("ALERT_TTL_SECONDS", "0"))

# How to deduplicate alerts: 'exact' (default), 'pair' (currency+method), 'currency' (currency only)
ALERT_DEDUP_MODE = os.getenv("ALERT_DEDUP_MODE", "exact").lower()

# Runtime behavior (headless, from env)
REFRESH_EVERY = int(os.getenv("REFRESH_EVERY", "60"))
Profit_Threshold_percent = float(os.getenv("PROFIT_THRESHOLD_PERCENT", "0.4"))

# Supported currencies and default payment methods map (same as original)
...
# Env variable to explicitly list pairs (optional).
# Format example: "EGP:InstaPay,Vodafonecash;GBP:SkrillMoneybookers"
PAIRS_ENV = os.getenv("PAIRS", "").strip()

# Alternative: select a single currency (or ALL). Used only if PAIRS_ENV not set.
SELECTED_CURRENCY = os.getenv("SELECTED_CURRENCY", "ALL").strip().upper()
SELECTED_METHOD = os.getenv("SELECTED_METHOD", "ALL").strip()

# MIN_LIMIT_THRESHOLDS env: e.g. "EGP=100;GBP=50;EUR=40" (optional)
MIN_LIMIT_THRESHOLDS_ENV = os.getenv("MIN_LIMIT_THRESHOLDS", "").strip()

# ---------------------- logging ----------------------
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# ---------------------- helpers: parse env inputs ----------------------
def parse_thresholds(env_str, defaults):
    """
    Parse strings like "EGP=100;GBP=50" into dict.
    If missing, return large default so that no unintended filtering happens.
    """
    ...
    for p in parts:
        if "=" in p or ":" in p:
            ...
            try:
                ...
                logging.warning(f"Cannot parse threshold value for {k}: {v}. Ignoring.")
        else:
...
def parse_pairs_env(env_str, thresholds, default_map):
    """
    Parse PAIRS env to produce list of (currency, method, threshold).
    env_str example: "EGP:InstaPay,Vodafonecash;GBP:SkrillMoneybookers"
    ...
    for g in groups:
        if ":" in g:
            ...
            # if a single value 'ALL' -> expand using default_map
...
if PAIRS_ENV:
    ...
    # use SELECTED_CURRENCY / SELECTED_METHOD
    if SELECTED_CURRENCY == "ALL":
        ...
        methods = payment_methods_map.get(cur, [])
        if not methods:
            logging.error(f"No known payment methods for selected currency {cur}. Exiting.")
            raise SystemExit(1)
        if SELECTED_METHOD and SELECTED_METHOD.upper() != "ALL":
            if SELECTED_METHOD not in methods:
                logging.error(f"Method {SELECTED_METHOD} is not in known methods for {cur}: {methods}. Exiting.")
                raise SystemExit(1)
...
if not pairs_to_monitor:
    logging.error("No currency/payment pairs selected via environment variables. Exiting.")
    raise SystemExit(1)
...
def fetch_page_raw(fiat, pay_type, trade_type, page, rows=ROWS_PER_REQUEST):
    ...
    try:
        ...
        logging.debug(f"Network/API error page={page} {fiat} {trade_type} {pay_type}: {e}")
        return []

# ---------------------- find first matching ad (stop early) ----------------------
...
# ---------------------- duplicate-alert protection (thread-safe) ----------------------
...
def is_duplicate_and_update(key):
    ...
    with recent_alerts_lock:
        ...
        if prev is None:
            ...
            return False
        if ALERT_TTL_SECONDS <= 0:
            # TTL <=0 means "suppress duplicates forever (until restart)" for the SAME key
            return True
...
def make_alert_key(mode, currency, variant, buy, sell, spread_percent):
    if mode == "currency":
        return f"{currency}"
    if mode == "pair":
        return f"{currency}|{variant}"
    # exact (default) — include price/spread to re-alert if numbers change
    return f"{currency}|{variant}|{buy['price']:.4f}|{sell['price']:.4f}|{spread_percent:.2f}"

# ---------------------- processing a single (currency, method) ----------------------
...
def process_pair(currency, method, threshold):
    ...
    for variant in variants:
        ...
        if not buy or not sell:
            logging.info(f"{currency} via {variant}: missing side (buy={bool(buy)} sell={bool(sell)}). Trying next variant if any.")
            continue

        try:
            spread_percent = ((buy["price"] / sell["price"]) - 1) * 100
        except Exception as e:
            logging.warning(f"Error computing spread for {currency} {variant}: {e}")
            continue
        ...
        if spread_percent >= Profit_Threshold_percent:
            ...
            if is_duplicate_and_update(alert_key):
                logging.info(f"Duplicate alert suppressed for {alert_key}")
            else:
                message = build_alert_message(currency, pay_friendly, buy, sell, spread_percent)
                send_telegram_alert(message)
        else:
            logging.debug(f"No alert: spread {spread_percent:.2f}% < threshold {Profit_Threshold_percent}%")

        break

# ---------------------- main loop ----------------------
logging.info(f"Monitoring pairs: {pairs_to_monitor}. Refresh every {REFRESH_EVERY}s")
logging.info(f"Profit threshold: {Profit_Threshold_percent}%. Alert dedup mode: {ALERT_DEDUP_MODE}. ALERT_TTL_SECONDS={ALERT_TTL_SECONDS}")

try:
    while True:
        start_ts = time.time()
        with ThreadPoolExecutor(max_workers=min(len(pairs_to_monitor), 6)) as ex:
            futures = [ex.submit(process_pair, cur, method, thr) for cur, method, thr in pairs_to_monitor]
            for f in futures:
                try:
                    f.result()
                except Exception as e:
        ...
        to_sleep = max(0, REFRESH_EVERY - elapsed)
        time.sleep(to_sleep)
